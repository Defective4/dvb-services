...
public class SettingsGenerator {
    ...
    public ServerSettings startInteractiveSetup() {
        ...
        defaults.server.bind.port = cli.ask(integer(1, Short.MAX_VALUE), defaults.server.bind.port, "Server bind port");
        defaults.server.baseURL = cli
                .ask(CLIValidators.URL, defaults.server.baseURL, "Server base URL used for access from outside.")
                .toString();
        ...
        while (true) {
            ...
            if (add) {
                ...
                ps.format = cli.ask(enumeration(MediaFormat.class), null,
                        "Media format of the playlist\n" + "Valid formats: " + Arrays.toString(MediaFormat.values()));
                playlists.add(ps);
            } else
                break;
        }
        ...
        defaults.server.formats = cli.ask(list(enumeration(MediaFormat.class)), null,
                "Comma-separated list of media formats enabled on this server.\n"
                        ...
                        + Arrays.toString(MediaFormat.values()));
        ...
        if (defaults.server.needsTranscoding()) {
            ...
            String params = cli.ask("-aq 2", "Additional command line parameters to pass to the audio converter.",
                    true);
        ...
        defaults.tools.streamProvider = cli.ask(t -> {
            ...
            return type;
                ...
                + Arrays.toString(Arrays.stream(ProviderType.values())
                        .filter(type -> type.isOfType(TransportStreamProvider.class)).toArray(ProviderType[]::new)));
